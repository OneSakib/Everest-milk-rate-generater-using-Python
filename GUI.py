        ...
        try:
            ...
            for snf in range(snfstart.get(), snfend.get() + 1):
                for fat in range(fatstart.get(), fatend.get() + 1):
                    ...
                    if fat < 66:
                        price = int((((snf * snfunit) + (fat * fatunit))/10+below65.get()) * 100)                        
                        if len(str(price))>4:
                            price = int((((snf * snfunit) + (fat * fatunit))/10+below65.get())*10)
                    if count == 3:
                        pricestring += '00\nj'
                        count = 0
                    if snf < 100:
...
def gui_main():
    ...
    fatendentry = Entry(root, font='TimesNewRoman 15 bold italic', textvariable=fatend, width=10,
                        bd=3)
    below65label = Label(root, text='Below 65: ', font='TimesNewRoman 15 bold italic', fg='blue')
    below65entry = Entry(root, font='TimesNewRoman 15 bold italic', textvariable=below65, width=10,
                        bd=3)
    above65label = Label(root, text='Above 65: ', font='TimesNewRoman 15 bold italic', fg='blue')
    above65entry = Entry(root, font='TimesNewRoman 15 bold italic', textvariable=above65, width=10,
                        bd=3)
    ...
    below65label.place(relx=0.5, rely=0.5)
    below65entry.place(relx=0.75, rely=0.5)
    above65label.place(relx=0.05, rely=0.6)
    above65entry.place(relx=0.3, rely=0.6)
